...
local State = {
    ...
    targetImages = {},
    AnimationCachePath = "PeppermintEmotes/AnimationCache.json",
    AnimationCache = {},
    AnimationListCachePath = "PeppermintEmotes/AnimationListCache.json",
    EmoteListCachePath = "PeppermintEmotes/EmoteListCache.json",
    CustomAnimationPath = "PeppermintEmotes/CustomAnimations.json",
    CustomAnimations = {},
    ...
    EmotePagePath = "PeppermintEmotes/EmotePages.json",
    EmotePages = {},
    currentEmotePageName = "Default",
    EmoteDataCachePath = "PeppermintEmotes/EmoteDataCache.json"
}
...
function saveAnimationCache()
    if writefile then
        pcall(function()
            if not isfolder("PeppermintEmotes") then makefolder("PeppermintEmotes") end
...
function SafeLoad(url, name)
    ...
    if not func then
        warn("PeppermintGrave | SafeLoad: Failed to parse " .. (name or "script") .. ": " .. tostring(err))
        return function() end
    end

    local ok, res = pcall(func)
    if not ok then
        warn("PeppermintGrave | SafeLoad: Error executing " .. (name or "script") .. ": " .. tostring(res))
        return function() end
    end
    return res
end

SafeLoad("https://raw.githubusercontent.com/PeppermintGrave/PeppermintEmotes/main/Dependencies/Notify.lua", "Notify System")
...
function GetAsset(asset, preloadedBytes)
    ...
    if assetStr:find("http") then
        ...
        local path = "PeppermintEmotes/Assets/" .. filename
        ...
        if not isfolder("PeppermintEmotes/Assets") then 
            pcall(function()
                if not isfolder("PeppermintEmotes") then makefolder("PeppermintEmotes") end
                makefolder("PeppermintEmotes/Assets") 
            end)
        end
...
local ConfigPath = "PeppermintEmotes/EmoteSettings.json"
...
local SettingsLib = SafeLoad("https://raw.githubusercontent.com/PeppermintGrave/PeppermintEmotes/main/Dependencies/Settings.lua", "Settings Library")
...
cleanDeletedFavorites = function()
    ...
    if totalChecks == 0 then
        getgenv().Notify({ Title = "PeppermintGrave | Clean", Content = "No favorites to check!", Duration = 3 })
        cleanFavCleaning = false
        resetCleanButton()
        return
    end

    getgenv().Notify({ Title = "PeppermintGrave | Clean", Content = "Checking " .. totalChecks .. " favorites...", Duration = 3 })
...
local ThemeConfigPath = "PeppermintEmotes/EmoteThemes.json"
...
function ApplyTheme(themeData)
    ...
    if not themeData then
        warn("PeppermintGrave | ApplyTheme: themeData is nil. Falling back to Default.")
    ...
    if not ok then
        warn("PeppermintGrave | ApplyTheme error: " .. tostring(err))
    end
end
...
State.enterCustomAnimationEditor = function(category, animName)
    ...
    if State.currentCustomAnimationName == "Default" then
        getgenv().Notify({ Title = "PeppermintGrave | Error", Content = "Cannot edit Default Animation set. Create a new one!", Duration = 3 })
        return
    end
    ...
    getgenv().Notify({ Title = "PeppermintGrave | Animation Editor", Content = "🖱️ Select an animation from the wheel to set for " .. animName, Duration = 5 })
end
...
SettingsLib.AddIconButton(CustomAnimMgtContainer, "78317476576895", function()
    ...
    imp.MouseButton1Click:Connect(function()
        ...
        if s and type(d) == "table" then
            ...
            if refreshCustomAnimationState then refreshCustomAnimationState(false) end
            popup:Destroy()
            getgenv().Notify({ Title = "PeppermintGrave | Animation", Content = "✅ Imported custom animations", Duration = 3 })
        else
            getgenv().Notify({ Title = "Error", Content = "Invalid JSON", Duration = 3 })
        end
    end)
...
SettingsLib.AddIconButton(EmotePageMgtContainer, "78317476576895", function() 
    ...
    imp.MouseButton1Click:Connect(function()
        ...
        if s and type(d) == "table" and d.Type == "EmotePageSet" and type(d.Data) == "table" then
            ...
            popup:Destroy()
            getgenv().Notify({ Title = "PeppermintGrave | Page", Content = "✅ Imported Emote page", Duration = 3 })
        else
        ...
        if s and type(d) == "table" and d.Type then
            ...
            if d.Themes and (typeStr == "All" or typeStr == "Themes") then
                ...
                if themeToApply then
                    ...
                    warn("PeppermintGrave | Missing Default theme during import fallback")
                end
            end
...
function disconnectAllConnections()
    ...
    if ContextActionService then
        ContextActionService:UnbindAction("PeppermintEmotes_EmoteWheelHotkeys")
    end
end
...
toggleFavorite = function(emoteId, emoteName)
    ...
    if found then
        ...
        getgenv().Notify({
            Title = 'PeppermintGrave | Favorite System',
...
applyAnimation = function(animationData)
    ...
    if not animate or not humanoid then
        getgenv().Notify({
            Title = 'PeppermintGrave | Animation Error',
    ...
    if index == 1 and randomActive then
        ...
        if State.customAnimationEditorActive then
            ...
            if State.CustomAnimations.Sets[State.currentCustomAnimationName] and cat and name then
                ...
                State.SaveCustomAnimations(State.CustomAnimations)
                getgenv().Notify({ Title = "PeppermintGrave | Saved", Content = "✅ Saved " .. name, Duration = 3 })
                if State.RefreshCustomAnimUI then State.RefreshCustomAnimUI() end
                if refreshCustomAnimationState then refreshCustomAnimationState(true) end
                State.exitCustomAnimationEditor()
            end
...
function fetchAllEmotes()
    ...
    local function fetchFromUrl()
        local success, result = pcall(function()
            local jsonContent = game:HttpGet("https://raw.githubusercontent.com/PeppermintGrave/PeppermintEmotes/main/Data/EmoteSniper.json")
    ...
    task.spawn(function()
        while true do
            ...
            if emoteData then
                ...
                getgenv().Notify({Title = 'PeppermintGrave | Emote', Content = "📦 Emotes loaded", Duration = 3})
                return
            end
            task.wait(3)
        end
...
function fetchAllAnimations()
    ...
    task.spawn(function()
        local success, result = pcall(function()
            local jsonContent = game:HttpGet("https://raw.githubusercontent.com/PeppermintGrave/PeppermintEmotes/main/Data/AnimationSniper.json")
            if jsonContent and jsonContent ~= "" then
                local data = HttpService:JSONDecode(jsonContent)
                return data.data or {}
            end
        ...
        if offsaleAnimationJson then
            offsaleSuccess, offsaleResult = pcall(function()
                local jsonContent = game:HttpGet("https://raw.githubusercontent.com/PeppermintGrave/PeppermintEmotes/main/Data/AnimationSniperoffsale.json")
                if jsonContent and jsonContent ~= "" then
                    local data = HttpService:JSONDecode(jsonContent)
                    return data.data or {}
                end
...
function searchAnimations(searchTerm)
    if State.isLoading then
        getgenv().Notify({
            Title = 'PeppermintGrave | Animation',
            Content = '⚠️ Loading please wait...',
            Duration = 5
        })
        return
...
function onCharacterAdded(character)
    ...
    if getgenv().autoReloadEnabled and getgenv().lastPlayedAnimation then
        task.spawn(function()
            ...
            getgenv().Notify({
                Title = 'PeppermintGrave | Auto Reload Animation',
...
function toggleEmoteWalk()
    ...
    if State.emotesWalkEnabled then
        getgenv().Notify({
            Title = 'PeppermintGrave | Emote Freeze',
...
function toggleSpeedEmote()
    ...
    if State.speedEmoteEnabled then
        getgenv().Notify({
            Title = 'PeppermintGrave | Speed Emote',
...
function connectEvents()
    ...
    local function bindWheelHotkeys()
        ...
        ContextActionService:BindActionAtPriority(
            "PeppermintEmotes_EmoteWheelHotkeys",
            onHotkey,
            false,
...
enterHUDEditor = function()
    ...
    local selectionGui = game:GetService("CoreGui"):FindFirstChild("PeppermintEmotes_HUDSelection")
    if not selectionGui then
        selectionGui = Instance.new("ScreenGui")
        selectionGui.Name = "PeppermintEmotes_HUDSelection"
    ...
    table.insert(HUD.Connections, lockBtn.MouseButton1Click:Connect(function()
        ...
        pcall(function() updateGUIColors() end)
        getgenv().Notify({ 
            Title = "PeppermintGrave | HUD Editor", 
    ...
    table.insert(HUD.Connections, exportBtn.MouseButton1Click:Connect(function()
        ...
        setclipboard(HttpService:JSONEncode(data))
        getgenv().Notify({ Title = "PeppermintGrave | HUD Editor", Content = "✅ HUD settings copied", Duration = 2 })
    end))
    ...
    table.insert(HUD.Connections, importBtn.MouseButton1Click:Connect(function()
        ...
        imp.MouseButton1Click:Connect(function()
            ...
            if s and type(d) == "table" then
                ...
                popup:Destroy()
                getgenv().Notify({ Title = "PeppermintGrave | HUD Editor", Content = "✅ HUD settings imported", Duration = 2 })
            else
                getgenv().Notify({ Title = "Error", Content = "Invalid HUD JSON", Duration = 3 })
            end
        end)
    ...
    table.insert(HUD.Connections, resetBtn.MouseButton1Click:Connect(function()
        ...
        getgenv().Notify({ Title = "PeppermintGrave | HUD Editor", Content = "🔄 All designs and frames have been fully reset", Duration = 3 })
    end))
    ...
    table.insert(HUD.Connections, deleteBtn.MouseButton1Click:Connect(function()
        ...
        ApplyUIVisibility()
        pcall(function() updateGUIColors() end)
        getgenv().Notify({ Title = "PeppermintGrave | HUD Editor", Content = "🗑️ Custom Frame deleted", Duration = 2 })
    end))
    ...
    table.insert(HUD.Connections, addBtn.MouseButton1Click:Connect(function()
        ...
        getgenv().Notify({ Title = "PeppermintGrave | HUD Editor", Content = "➕ Custom Frame added!", Duration = 2 })
    end))

    getgenv().Notify({ Title = "PeppermintGrave | HUD Editor", Content = "✏️ Drag elements to reposition", Duration = 5 })
end
...
if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled then
    SafeLoad("https://raw.githubusercontent.com/PeppermintGrave/PeppermintEmotes/main/Dependencies/OpenEmote.lua", "Open Emote")
    getgenv().Notify({
        Title = 'PeppermintGrave | Emote Mobile',
...
if UserInputService.KeyboardEnabled then
    getgenv().Notify({
        Title = 'PeppermintGrave | Emote PC',
